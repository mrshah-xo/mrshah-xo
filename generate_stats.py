...
if not USERNAME or USERNAME == "your-username":
    sys.exit(
        f"Set GITHUB_USERNAME in {CONFIG_PATH} to your real GitHub username"
    )


EXCLUDE_REPOS = [f"{USERNAME}/{USERNAME}"] + [
    r.strip()
    for r in os.environ.get("EXCLUDE_REPOS", "").split(",")
    if r.strip()
]

EXCLUDE_FORKS = (
    os.environ.get("EXCLUDE_FORKS", "true").lower() != "false"
)
...
def get_lines_of_code(repos):
    ...
    for repo in repos:
        ...
        for attempt in range(6):
            r = SESSION.get(
                f"{API}/repos/{full_name}/stats/contributors"
            )
            ...
            # FIX:
            # GitHub can return "author": null.
            # Therefore we safely convert None into an empty dictionary.
            for entry in r.json() or []:
                ...
                # "weeks" can also theoretically be missing/null.
                for week in entry.get("weeks") or []:
                    additions += week.get("a", 0) or 0
                    deletions += week.get("d", 0) or 0
...
def main():
    ...
    print(
        f"  owned repos counted: {len(owned_repos)}"
    )
    ...
    # Personal/content fields from config/profile.env.
    # Escaped so stray &, <, > in someone's text can't break
    # the SVG's XML.
    values = {
        k: html.escape(
            v,
    ...
    for theme in ("dark", "light"):
        with open(
            f"templates/overview_{theme}.svg",
            encoding="utf-8",
        ) as f:
            svg = f.read()
