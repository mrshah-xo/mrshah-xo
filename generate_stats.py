        ...
        for attempt in range(6):
            ...
            for entry in r.json() or []:
                if not isinstance(entry, dict):
                    continue
                # GitHub can return an entry with "author": null for
                # commits from deleted or unlinked accounts - .get()
                # only falls back to {} when the key is *missing*, not
                # when it's present but None, so that case needs an
                # explicit check.
                ...
                if login.lower() == USERNAME.lower():
                    for week in entry.get("weeks") or []:
                        if not isinstance(week, dict):
                            continue
                        additions += week.get("a") or 0
                        deletions += week.get("d") or 0
            break
